...
class MY_Session extends CI_Session
{
	...
	{
	  if ( $name ) return md5($this->sess_cookie_name);
	...
	/**
		...
		{
			$this->sess_destroy();
			log_message('debug', 'Session data is incorrect format');
			return FALSE;
		}
		...
		{
			$this->sess_destroy();
			log_message('debug', 'Session has expired');
			return FALSE;
		}
		...
		{
			$this->sess_destroy();
			log_message('debug', 'Session IP match fail');
			return FALSE;
		}
		
		// Disabled User Agent Match for Flash
		...
		if ($this->sess_use_database === TRUE)
		{
			...
			{
				$this->sess_destroy();
				log_message('debug', 'Session data not found in database');
				return FALSE;
			}
