...
class Migration_Update009 extends CI_Migration {
	...
	function up() {
		...
		$this->db->query("
				ALTER TABLE `" . $this->db->dbprefix('comics') . "` ADD `altname` VARCHAR( 512 ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL AFTER `name`
		");
		$this->db->query("
				ALTER TABLE `" . $this->db->dbprefix('comics') . "` ADD `genres` VARCHAR( 512 ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL AFTER `description`
		");
		$this->db->query("
