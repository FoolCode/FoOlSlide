...
class Migration_Update009 extends CI_Migration {
	...
	function up() {
		...
		");
		$this->db->query("
				ALTER TABLE `" . $this->db->dbprefix('comics') . "` ADD `publisher` ENUM('Weekly Shounen Jump','Weekly Shounen Magazine','Weekly Shounen Sunday','Bessatsu Shounen Magazine','Ultra Jump','Shounen A','Morning','Beans Ace','Shuukan Shounen Champion','Shounen Gangan','Comic Blade','Young King Ours','Jump SQ','Monthly Shounen Magazine','Champion RED','Dengeki Daioh') NOT NULL AFTER `artist`
		");
	}
